class ProGrids_Base extends EditorWindow 
{
	...
	function Update()
	{
		if(!proGrids || !proGrids.snapToGrid || Application.isPlaying)
			return;			

		if(Selection.transforms.Length > 0) //if grid is found
		{
			if(lastActiveTransform != Selection.transforms[0] && !ContainsTransform(Selection.transforms, lastActiveTransform)) {
				lastActiveTransform = Selection.activeTransform;
				activeTransformPos = lastActiveTransform.position;
			}

			if(lastActiveTransform.position != activeTransformPos)
				DoSnap();
	...
	*/
	...
	{             
		...
		var old : Vector3 = lastActiveTransform.position;
		lastActiveTransform.position = FindNearestSnapPos(old);
		var offset : Vector3 = lastActiveTransform.position - old;

		for(var i = 0; i < Selection.transforms.Length; i++)
		{
			if(Selection.gameObjects[i].GetComponent(MeshFilter) && Selection.transforms[i] != lastActiveTransform)
			{
	...
	function ContainsTransform(t_arr : Transform[], tr : Transform) : boolean
	{
		for(var t : Transform in t_arr)
		{
			if(t == tr)
				return true;
		}
